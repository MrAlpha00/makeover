...
export default function SubcategoryPage({ params }) {
  ...
  // Find category and subcategory for display names
    ...
    return (
      <div className="min-h-screen bg-dark-900 flex flex-col">
        ...
        <main className="flex-1 flex flex-col items-center justify-center text-center px-4 pt-20">
          ...
          <p className="text-white/50 text-xl mb-8">Subcategory not found</p>
          <Link href={`/${category}`} className="bg-coral-500 hover:bg-coral-600 text-white px-8 py-3 rounded-full font-medium transition-colors">
            Back to {categoryData ? categoryData.name : 'Categories'}
          </Link>
        </main>
        <Footer />
      </div>
  ...
  const sortedAndFilteredServices = useMemo(() => {
    let filtered = services.filter(s => 
      s.categorySlug === category && s.subcategorySlug === subcategory
    );
    ...
    return filtered.sort((a, b) => {
      if (sortOrder === 'price-low') {
        ...
        // Fallback for newest (just reversed id or rating)
      ...
      <div className="sticky top-[60px] sm:top-[72px] z-30 bg-dark-900/95 backdrop-blur-md border-b border-white/5 py-4">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center text-sm font-medium text-white/50 space-x-2">
            ...
            <Link href={`/${category}`} className="hover:text-coral-400 transition-colors">{categoryData.name}</Link>
            <ChevronRight className="w-3 h-3" />
            <span className="text-white/90">{subcategoryData.name}</span>
          </div>
          ...
          <div className="flex flex-row items-center justify-between w-full sm:w-auto gap-4">
            ...
            <div className="relative">
              <button 
                ...
                className="flex items-center gap-2 text-sm bg-dark-800 border border-white/10 text-white px-4 py-2 rounded-full hover:border-coral-500/50 transition-colors relative z-40 relative"
              >
        ...
        <div className="max-w-7xl mx-auto">
          {sortedAndFilteredServices.length === 0 ? (
            <div className="text-center py-20 bg-dark-800/20 rounded-3xl border border-white/5">
              ...
              <Link href={`/${category}`} className="mt-4 inline-block text-coral-400 hover:text-coral-300">
              ...
              {sortedAndFilteredServices.map((service) => (
                ...
                >
                  ...
                  <div className="relative aspect-[4/3] w-full overflow-hidden">
                    ...
                    <img 
                      src={`/assets/designs/${service.id}.jpg`} 
                      onError={(e) => { e.currentTarget.src = service.image; e.currentTarget.onerror = null; }}
                      alt={service.title} 
                  ...
                  <div className="p-5 flex-1 flex flex-col">
                    ...
                    <p className="text-white/50 text-sm mb-4 line-clamp-1 flex-1">{service.shortDesc}</p>
