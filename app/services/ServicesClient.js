...
export default function ServicesClient({ initialDesigns, parentCategories }) {
  ...
  return (
    <>
      {/* Filters */}
      <div className="sticky top-16 z-30 bg-light-primary/95 dark:bg-dark-900/95 backdrop-blur-md border-y border-light-border dark:border-white/5 py-4">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex flex-col sm:flex-row items-start sm:items-center gap-4">
            ...
            <button
              ...
              className={`text-sm px-4 py-1.5 rounded-full border transition-all font-medium ${
                activeCategory === 'All'
                  ...
                  : 'border-light-border dark:border-white/10 text-light-text-secondary dark:text-white/50 hover:border-coral-500/50 hover:text-light-text-primary dark:hover:text-white'
              }`}
            >
              All
            </button>
          ...
          <div className="relative sm:ml-auto flex-shrink-0 w-full sm:w-auto">
            <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-light-text-secondary dark:text-white/30" />
            <input
              type="text"
              placeholder="Search decorations..."
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="input-light dark:input-dark pl-9 w-full sm:w-60 py-2"
            />
          </div>
        </div>
      </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 py-12 min-h-[40vh]">
        {filtered.length === 0 ? (
          <div className="text-center py-20">
            <p className="text-light-text-secondary dark:text-white/30 text-lg">No services found. Try a different filter.</p>
          </div>
        ) : (
