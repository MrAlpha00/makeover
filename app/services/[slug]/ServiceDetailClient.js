...
export default function ServiceDetailClient({ service, related, alsoBooked }) {
  ...
  const gallery = service.images?.length ? service.images : (service.image ? [service.image] : ['/assets/placeholder.jpg']);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-28 pb-20">
      ...
      {related && related.length > 0 && (
        <div className="mt-16">
          <h2 className="section-title mb-8">More <span className="italic">{service.categories?.name || 'similar'}</span> setups</h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {related.map((s, i) => (
              <ServiceCard key={s.id} service={s} index={i} />
            ))}
